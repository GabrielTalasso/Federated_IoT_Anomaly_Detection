...
class CentralizedClientFlower(fl.client.NumPyClient):
	...
	def __init__(self, cid, dataset, model_name, anomaly_round, n_clients, model_shared = 'All', loss_type = 'mse',
		...
		self.x_train, self.x_test= self.load_data(server_round=1, cid = self.cid)
	...
	def load_data(self, server_round, cid, dataset_size = 60):
		x_train, x_test = load_dataset(dataset_name=self.dataset, cid = cid, n_clients = self.n_clients,
